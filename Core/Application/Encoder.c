...
int sevenway_data;
int32_t encoder_total_l;  // 累计脉冲总数
int32_t encoder_total_r;  // 累计脉冲总数
int left_speed, right_speed;
float x_data, y_data, z_data;
int32_t target_pulses = 0;           // 目标脉冲数
uint32_t tim6_at;
...
void int16_to_3uint8(int16_t value, uint8_t* sign, uint8_t* high, uint8_t* low)
{
    ...
    *sign = (value >> 15) & 0x01;
    ...
    // 3. 分离高7位和低8位
    *high = (abs_value >> 8) & 0x7F; // 取高7位（掩码0x7F过滤符号位）
    *low  = abs_value & 0xFF;        // 取低8位
}
// 将两个int16_t存入uint8_t数组
...
void uint8_to_bin_str(uint8_t value, char* buffer)
{
    ...
    {
        // 从高位到低位依次检查每一位是否为1
        buffer[i] = (value & (0x80 >> i)) ? '1' : '0'; // 0x80对应二进制10000000
    }
    buffer[8] = '\0'; // 添加字符串结束符
}
...
void Encoder_Get(void)
{
    ...
    encoderLeft = __HAL_TIM_GetCounter(&htim2);//获取计数值
    //encoder_total_l += encoderLeft;
    __HAL_TIM_SetCounter(&htim2, 0);//清空计数值

    encoderRight = __HAL_TIM_GetCounter(&htim3);//获取计数值
...
void StartRotation(int turns, int speed, uint8_t left_or_right)
{
    ...
    {
        encoder_total_l = 0;
        while(abs(encoder_total_l) < abs(target_pulses))motor_pid_control(speed * turns, 0);
        MG513_SetLeft(0);
    }
    else if(left_or_right == 2)
    {
        encoder_total_r = 0;
        while(abs(encoder_total_r) < abs(target_pulses))motor_pid_control(0, speed * turns);
        MG513_SetRight(0);
    }
    else
    {
        encoder_total_r = 0;
        encoder_total_l = 0;
        while((abs(encoder_total_l) + abs(encoder_total_r)) / 2 < abs(target_pulses))motor_pid_control(speed * turns, speed * turns);
        MG513_Set(0, 0);
    }
...
void mpu_0p(void)
{
    ...
    while(1)
    {
        if(receive_flag)
        {   
					  if(fabsf(prev-z)<0.0001&&js>50)break;
					  receive_flag=0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* htim)
{
    ...
    else if(htim == &htim3)  //编码器
    {
    }
    else if(htim == &htim6) //计算转速20ms
    {
        //mpu_dmp_get_data(&x_data,&y_data,&z_data);


        Encoder_Get();
        //sevenway_data = SevenWay_Read();
        //error_calculate();
        mpu_dmp_get_data(&x_data, &y_data, &z);
        tim6_at++;
			  //z=0.9*z+0.1*fter;
			  if(fabsf(prev-z)<0.15f)offset=offset+prev-z;
			  z_data=Normalization(z+offset);//+fminf(8,tim6_at*0.01150f);//+fminf(30,tim6_at*0.045);
			  
        motor_pid_control(left_speed, right_speed);
        //
			
        if(!receive_flag)receive_flag = 1;

        sprintf(k, "%d  %d  ", encoderLeft, -encoderRight);
        OLED_ShowString(0, 0, k, OLED_8X16);
//			  sprintf(k, "%x ",seven_ff);
//			  OLED_ShowString(0, 16, k, OLED_8X16);
			  ...
			  OLED_ShowFloatNum(0,32,target_angle,3,3,OLED_8X16);
				...
				//OLED_ShowFloatNum(40,48,offset,2,3,OLED_8X16);
//        sprintf(k, "%d ", uwTick/1000);
