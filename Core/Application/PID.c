...
float Normalization(float data)
{
    ...
    if(new_angle > 180.0f) new_angle -= 360.0f;
    else if(new_angle <= -180.0f) new_angle += 360.0f;
    return new_angle;
}

float constrain(float constrained, float max_up, float max_down)
{
    if(constrained > max_up)return max_up;
    if(constrained < max_down)return max_down;
    return constrained;
}
...
float PID_Update(PIDController* pid, float setpoint, float input)
{
    ...
    // 3. 改进的微分项（基于输入变化率，降低设定值突变影响）
    float derivative = (input - pid->prev_input) / dt;

    // 4. 微分项低通滤波（一阶滞后滤波）
    pid->derivative_filtered = pid->filter_coef * pid->derivative_filtered
                               + (1 - pid->filter_coef) * derivative;

    // 5. 计算输出
    float output = pid->Kp * error
                   + pid->Ki * pid->integral
                   - pid->Kd * pid->derivative_filtered; // 注意符号

    // 6. 输出限幅及抗饱和处理
    if(output > pid->output_limit)
    {
        output = pid->output_limit;
        pid->integral -= error * dt; // Clamping抗饱和
    }
...
void seven_line_pid_control(int speed)
{
    ...
    int out;
    float data;
    if(count_zeros_8bit(seven_ff) > 4)return;
...
void length_control(int left, int right)
{
    ...
    while(1)
    {
        if((abs(left) - abs(encoder_total_l)) <= 0 && (abs(right) - abs(encoder_total_r)) <= 0)break;
...
//	if(error<-180)error=error+360;
    ...
    // z_data = z + fmin(tim6_at / 50, 15) + 0.49f; //零漂处理



//	  target=target_angle-sevenway_data*0.8;//根据7路偏移量改变目标角度
...
void mpu6050_turn_angle(float target, uint8_t way)
{
    ...
    data_1 = (int)PID_Update(&mpu6050_pid, 0, -error);
    // error_calculate();
	 if(fabsf(error)<5)motor_pid_init();
    if(data_1 > 0)
    {
...
{
    // 200: PID_Init(&ms_c, 0.25f, 0.03f, 0.05f, 45.0f, 3000.0f, 0.02f); // 启用完整PID
...
void mpu6050_sevenway_control(int speed)
{
    ...
    if(speed > 70)
    {
        ...
        if(fabsf(delta) >= 24)ms_c.Kp = 0.8f;
        else if(fabsf(delta) >= 19)ms_c.Kp = 0.6f;
        else if(fabsf(delta) >= 9)ms_c.Kp = 0.5f;
        else ms_c.Kp = 0.5f;
    }
    else
    {
...
//	  if(fabsf(prev_suint8evenway-current_angle)!=0) ms_c.integral=0;
    ...
    OLED_ShowFloatNum(90, 48, delta, 2, 2, OLED_8X16);
		...
		if(receive_flag){receive_flag=0;mpu6050_pid_control(speed, target);}
#else
    if(receive_flag)
    {
        receive_flag = 0;
...
#define HISTORY_SIZE 128  // 历史数据缓冲区大小（与屏幕宽度一致）
static int16_t history[HISTORY_SIZE] = {0}; // 存储偏差值的历史数据
static uint8_t data_index = 0;              // 最新数据位置
static float max_value = 10.0f;            // 纵轴最大值（自动调整）

/**
  * @brief  显示目标值与偏差的动态曲线
  * @param  target 目标值（范围：-1000~1000）
  * @param  deviation 偏差值（范围：-1000~1000）
  * @retval 无
  */
void OLED_ShowDynamicData(int16_t target, int16_t deviation)
{
    ...
    float current_max = fabsf(deviation * 1.2f); // 留20%余量
    max_value = (current_max > prev_max) ? current_max : prev_max * 0.95f;
    prev_max = max_value;

    // 4. 绘制目标参考线（红色）
    uint8_t target_y = 16 + (int)((target / max_value) * 24);
    target_y = (target_y > 63) ? 63 : (target_y < 16) ? 16 : target_y;
    for(int x = 0; x < 128; x++)
    {
        OLED_DrawPoint(x, target_y);  // 绘制红线（需配合反色模式）
    }
    ...
    for(int i = 0; i < HISTORY_SIZE; i++)
    {
        ...
        uint8_t y = 40 - (ratio * 24); // 16~40像素为曲线区域
        y = (y < 16) ? 16 : (y > 40) ? 40 : y;
        OLED_DrawPoint(i, y);          // 绘制数据点
    }
